...
mapping = {"cifar10":            {"resnet18"  : cifar10_server_garbler_utils_r18,
...
parser.add_argument("--client-nonlinear",      type=float, default=18.248,     help="amount of nonlinear storage required by client (KB/ReLU)")
parser.add_argument("--server-nonlinear",      type=float, default=3.59,       help="amount of nonlinear storage required by client (KB/ReLU)")
...
def run_online_only(env, pipe):
    ...
    while True:
        ...
        yield env.timeout(gc_models.garble_latency("server", measurements.NUM_RELU, args.num_threads_server_gc)) 
        yield env.timeout(measurements.off_server_compute_encode)
        yield env.timeout(measurements.off_server_write_garbled_c / download_bandwidth)          # client sends garbled circuit to server

        yield env.timeout(measurements.off_client_write_base_ot / upload_bandwidth)             # server sends labels (k_0, k_1)..... BASE OT
        yield env.timeout(measurements.off_client_write_ext_ot_setup / upload_bandwidth)
        yield env.timeout(measurements.off_server_write_ext_ot_send / download_bandwidth)



        yield env.timeout(measurements.on_client_write_linear.sum() / upload_bandwidth)
        yield env.timeout(measurements.on_server_compute_encode.sum())
        yield env.timeout(measurements.on_server_write_relu.sum() / download_bandwidth)

        # evaluate linear layers using secret sharing on the server
        yield env.timeout(measurements.on_server_compute_linear.sum())

        # evaulate Garbled Circuits on the client device
        yield env.timeout(gc_models.eval_latency("client", measurements.NUM_RELU, args.num_threads_client))
        yield env.timeout(measurements.on_server_write_pred / download_bandwidth)
    ...
    while True:
        ...
        if (client_storage.capacity - client_storage.level) > client_storage_per_inf:
            if (server_storage.capacity - server_storage.level) > server_storage_per_inf:
                yield env.process(offline_server_garbler_phase(env, client_storage, server_storage))


def offline_server_garbler_phase(env, client_storage, server_storage):
    """
...
def online_server_garbler_phase(env, pipe, client_storage, server_storage):
    """
...
for exp_number in tqdm(range(args.number_of_runs)):
    for j in range(len(arrival_rates)):
        ...
        if client_storage_per_inf < CLIENT_STORAGE:
            ...
            env.process(online_server_garbler_phase(env, pipe, client_storage, server_storage))
        else:
...
np.save("{}/server_garbler_avg_inf_times.npy".format(args.name), avg_inf_times)
np.save("{}/server_garbler_arrival_rates.npy".format(args.name), arrival_rates )
np.save("{}/server_garbler_total_num_clients.npy".format(args.name), total_num_clients)
np.save("{}/server_garbler_avg_waiting_times.npy".format(args.name), avg_waiting_times)
np.save("{}/server_garbler_left_to_service.npy".format(args.name), left_to_service)
if client_storage_per_inf < CLIENT_STORAGE:
    np.save("{}/server_garbler_avg_offline_times.npy".format(args.name), avg_offline_times)
